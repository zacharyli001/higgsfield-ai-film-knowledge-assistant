...
const transientSpeechError=error=>/operation could not be completed|no speech|没有听到|没有识别|1101|203|216/i.test(String(error?.message||error||''));
...
chrome.runtime.onMessage.addListener((msg,sender,reply)=>{
  ...
  if(msg?.type==='hf-offscreen-audio-chunk'){
    ...
    configWith().then(async config=>{const source=config.subtitleRecognizer==='apple'?await transcribeApple(msg.dataUrl):await transcribeAudio(msg.dataUrl,config),mode=config.subtitleOutputMode||'chinese';if(mode==='english'){await chrome.tabs.sendMessage(msg.tabId,{type:'hf-subtitle-update',text:source,source,sequence:msg.sequence,interim:false,outputMode:mode});return {ok:true,englishOnly:true};}if(mode==='hybrid')await chrome.tabs.sendMessage(msg.tabId,{type:'hf-subtitle-update',text:source,source,sequence:msg.sequence,interim:true,outputMode:mode});const text=await translateSubtitle(source,config);await chrome.tabs.sendMessage(msg.tabId,{type:'hf-subtitle-update',text,source,sequence:msg.sequence,interim:false,outputMode:mode});return {ok:true};}).then(reply,error=>{if(transientSpeechError(error)){reply({ok:false,skipped:true,error:error.message});return;}chrome.tabs.sendMessage(msg.tabId,{type:'hf-subtitle-status',text:`字幕错误：${error.message}`,sequence:msg.sequence}).catch(()=>{});reply({ok:false,error:error.message});});return true;
  }
