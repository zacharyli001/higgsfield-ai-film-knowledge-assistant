...
async function transcribeApple(dataUrl){
  ...
  let lastError;for(let attempt=0;attempt<2;attempt++){try{const result=await nativeRequest({action:'transcribe',audio:match[2],locale:'en-US'},35000),text=String(result.text||'').trim();if(!text)throw new Error('Apple 本地识别没有听到英文');return text;}catch(error){lastError=error;if(!transientSpeechError(error)||attempt)throw error;await new Promise(resolve=>setTimeout(resolve,160));}}throw lastError;
}
let appleRecognitionTail=Promise.resolve();
function transcribeAppleQueued(dataUrl){
  const job=appleRecognitionTail.catch(()=>{}).then(()=>transcribeApple(dataUrl));
  appleRecognitionTail=job.catch(()=>{});
  return job;
}
...
async function subtitleFromAudio(dataUrl,config){const source=config.subtitleRecognizer==='apple'?await transcribeAppleQueued(dataUrl):await transcribeAudio(dataUrl,config);return {source,text:await translateSubtitle(source,config)};}
...
chrome.runtime.onMessage.addListener((msg,sender,reply)=>{
  ...
  if(msg?.type==='hf-offscreen-audio-chunk'){
    ...
    configWith().then(async config=>{const source=config.subtitleRecognizer==='apple'?await transcribeAppleQueued(msg.dataUrl):await transcribeAudio(msg.dataUrl,config),mode=config.subtitleOutputMode||'chinese';if(mode==='english'){await chrome.tabs.sendMessage(msg.tabId,{type:'hf-subtitle-update',text:source,source,sequence:msg.sequence,interim:false,outputMode:mode});return {ok:true,englishOnly:true};}if(mode==='hybrid')await chrome.tabs.sendMessage(msg.tabId,{type:'hf-subtitle-update',text:source,source,sequence:msg.sequence,interim:true,outputMode:mode});const text=await translateSubtitle(source,config);await chrome.tabs.sendMessage(msg.tabId,{type:'hf-subtitle-update',text,source,sequence:msg.sequence,interim:false,outputMode:mode});return {ok:true};}).then(reply,error=>{if(transientSpeechError(error)){reply({ok:false,skipped:true,error:error.message});return;}chrome.tabs.sendMessage(msg.tabId,{type:'hf-subtitle-status',text:`字幕错误：${error.message}`,sequence:msg.sequence}).catch(()=>{});reply({ok:false,error:error.message});});return true;
  }
