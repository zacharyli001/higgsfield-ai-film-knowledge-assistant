...
async function start(streamId,tabId){
  await stop();
  const stored=await chrome.storage.local.get({subtitleConcurrency:4}),concurrency=Math.max(1,Math.min(6,Number(stored.subtitleConcurrency)||4));
  ...
  const state=session={active:true,tabId,stream,context,source,analyser,processor,silent,samples,queue,running:0,concurrency,sequence:0,pcm:[],started:performance.now(),speech:false,silence:0,monitor:null};
  const process=()=>{while(state.active&&state.running<state.concurrency&&state.queue.length){const item=state.queue.shift();state.running++;(async()=>{try{await chrome.runtime.sendMessage({type:'hf-offscreen-audio-chunk',tabId,sequence:item.sequence,dataUrl:await asDataUrl(item.blob)});}finally{state.running--;process();}})();}};
  ...
  const cut=()=>{const heardSpeech=state.speech,blob=heardSpeech?wav(state.pcm):null;state.pcm=[];state.started=performance.now();state.speech=false;state.silence=0;if(blob&&blob.size>1000){if(state.queue.length>=state.concurrency)state.queue.shift();state.queue.push({blob,sequence:++state.sequence});process();}};
